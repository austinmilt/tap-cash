
//TODO figure out how to expose these via env files

// use http://10.0.2.2:8080/ when testing/deploying with android emulator locally

//TODO
export const NEW_MEMBER_URI: string = "https://list-channels-7pi36k236a-uc.a.run.app";
export const DEPOSIT_URI: string = "https://list-channels-7pi36k236a-uc.a.run.app";
export const SEND_URI: string = "https://list-channels-7pi36k236a-uc.a.run.app";
export const WITHDRAW_URI: string = "https://list-channels-7pi36k236a-uc.a.run.app";
export const QUERY_RECIPIENTS_URI: string = "https://query-recipients-7pi36k236a-uc.a.run.app";
export const RECENT_ACTIVITY_URI: string = "https://list-channels-7pi36k236a-uc.a.run.app";
export const SAVED_PAYMENT_METHODS_URI: string = "https://list-channels-7pi36k236a-uc.a.run.app";
